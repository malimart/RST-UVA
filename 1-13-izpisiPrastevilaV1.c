...
int main()
{
    int n = 100;                        // izberi stevilo n
    int i, j, flag = 0;
    for (i = 2; i < n; i++)             // s prvo zanko se sprehodimo po vseh stevilih < n
    {
        for (j = 2; j <= i / 2; j++)    // z drugo zanko za vsako trenutno stevilo "i" preverimo koliko ima deliteljev
        {
            if (i % j == 0)
            {
